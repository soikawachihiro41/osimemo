class MypagesController < ApplicationController
  ...
  def tag
    ...
    @photos = Photo.joins(:tags, :album).where(tags: { tag_names: params[:tag] }).where("albums.id IN (:public_album_ids) OR photos.uploader_id = :user_id", public_album_ids: public_album_ids, user_id: current_user.id)
